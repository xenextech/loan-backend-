...
export class DashboardApplicationsController {
  ...
  @ApiOperation({
    summary:
      'List submitted applications for credit-ops (paginated) — ref no, borrower, branch, type, amount, grade, stage, DSGIR, LTV, days open',
  })
  list(@Query() query: DashboardApplicationsQueryDto) {
    return this.dashboardApplicationsService.list(query);
  }
