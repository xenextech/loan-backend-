...
export class OfferLetterService {
  ...
  async create(dto: CreateOfferLetterDto, email: string) {
    const { college, document, student, program, fees, conditions, signatories, qr } = dto;
    ...
    return this.prisma.offerLetter.create({
      ...
      data: {
        ...
        citizenshipNo:    student.citizenshipNo,
  ...
  async update(id: string, dto: UpdateOfferLetterDto, email: string) {
    ...
    return this.prisma.offerLetter.update({
      ...
      data: {
        ...
        ...(student?.citizenshipNo    && { citizenshipNo:    student.citizenshipNo }),
        ...
        ...(program?.programFullName    && { programFullName:    program.programFullName }),
        ...(program?.programAffiliation && { programAffiliation: program.programAffiliation }),
        ...(program?.durationYears      && { durationYears:      program.durationYears }),
        ...(program?.totalSemesters     && { totalSemesters:     program.totalSemesters }),
        ...(program?.creditHours        && { creditHours:        program.creditHours }),
        ...(program?.academicYearBS     && { academicYearBS:     program.academicYearBS }),
        ...(program?.intakeMonthBS      && { intakeMonthBS:      program.intakeMonthBS }),
        // Fees
        ...(fees?.admissionFee  !== undefined && { admissionFee:  fees.admissionFee }),
        ...(fees?.tuitionPerSem !== undefined && { tuitionPerSem: fees.tuitionPerSem }),
        ...(fees?.examFeePerSem !== undefined && { examFeePerSem: fees.examFeePerSem }),
        ...(fees?.labFeePerSem  !== undefined && { labFeePerSem:  fees.labFeePerSem }),
        ...(fees?.totalApprox   !== undefined && { totalApprox:   fees.totalApprox }),
        // JSON arrays
        ...(conditions  !== undefined && { conditions:  conditions  as unknown as Prisma.InputJsonValue }),
        ...(signatories !== undefined && { signatories: signatories as unknown as Prisma.InputJsonValue }),
        // QR
